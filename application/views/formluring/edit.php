      ...
      <div class="card card-primary">
        ...
        <input type="hidden" name="pelatihan_id" required readonly value="<?= $row->pelatihan_id ?>">
          <div class="card-body">
            ...
            <div class="form-group">
              ...
              <div>
                <select name="status_usaha" class="form-control" required>
                  <option value="<?= $this->input->post('status_usaha') ?? $row->status_usaha ?>">Pilih : <?= $this->input->post('status_usaha') ?? $row->status_usaha ?></option>                
            ...
            <?php if($row->ktp != null) {?>
            <div>
              <img src="<?=base_url('assets/dist/files/formluring/ktp/'.$row->ktp)?>" style="width: 30%"><br>
              <input type="hidden" name="ktp" value="<?= $this->input->post('ktp') ?? $row->ktp; ?>">
              <a href="<?= site_url('formluring/hapusktp/'.$row->id);?>" class="btn btn-sm btn-warning" onclick="return confirm('Apakah yakin mau dihapus?')"><small>Hapus ktp?</small></a> 
            </div>
            <?php } else {  ?> 
            <div class="form-group">
            ...
            <?php if($row->spt != null) {?>
            <div>
              <h4>spt : <a href="<?= base_url('assets/dist/files/formluring/spt/'.$row->spt)?>" class="text-primary"><?= $this->input->post('spt') ?? $row->spt; ?></h4>
              <input type="hidden" name="spt" value="<?= $this->input->post('spt') ?? $row->spt; ?>">
              <a href="<?= site_url('formluring/hapusspt/'.$row->id);?>" class="btn btn-sm btn-warning" onclick="return confirm('Apakah yakin mau dihapus?')"><small>Hapus spt?</small></a> 
            </div>
            <?php } else {  ?>
            <div class="form-group">
